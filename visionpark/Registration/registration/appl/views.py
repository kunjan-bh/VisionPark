...
def signuppage(request):
    if request.method == 'POST':
        ...
        my_user.save()
        phone_numbers.objects.create(user=my_user, phone=phone)
    ...
    if request.method == 'POST':
        ...
        if extend_time:
            ...
            context = {
                ...
                'new_totalendtime': new_totalendtime
            }
            return render(request, 'extend_esewa_pay.html', context)
        return redirect('book')
    
@login_required
def confirm_extend(request):
    user_id = request.GET.get('user_id')
    booking_id = request.GET.get('booking_id')
    extendTime = request.GET.get('extend_time')
    # new_endtime_str = request.GET.get('new_endtime')
    # new_totalendtime_str = request.GET.get('new_totalendtime')
    # new_endtime = datetime.strptime(new_endtime_str, "%b. %d, %Y, %I:%M %p")
    # new_totalendtime = datetime.strptime(new_totalendtime_str, "%b. %d, %Y, %I:%M %p")
    # new_endtime_str_formatted = new_endtime.strftime("%Y-%m-%d %H:%M:%S")
    # new_totalendtime_str_formatted = new_totalendtime.strftime("%Y-%m-%d %H:%M:%S")
    user = get_object_or_404(User, id=int(user_id))
    booking = get_object_or_404(Booking, id=int(booking_id), user=user)
    if booking:
        new_endtime = booking.end_time + timedelta(minutes=int(extendTime))
        new_totalendtime = booking.total_endtime + timedelta(minutes=int(extendTime))
...
@login_required
def extend_epay(request):
    return render(request, 'extend_esewa_pay.html')
    
@login_required
def bookfixed(request):##################################current work
    current_user = request.user
    full = current_user.first_name + ' ' + current_user.last_name
    phoneno = get_object_or_404(phone_numbers, user = current_user)
    ...
    if request.method == 'POST':
        ...
        existing_booking1flex = Booking.objects.filter(
            selected_slot=selected_slot,
            start_time__lt=end_datetime ,
            total_endtime = datetime.combine(datetime.today(), time(0, 0, 0)),
            booking_type = 'flexible',
            start_time__date__gte=timezone.now().date()
        ).exists()
        if existing_booking1 or existing_booking1flex:
        ...
        return render(request, 'payment_esewa.html', context)
    default_context = {
        'default_name': full,
        'number': number
    }
    return render(request, 'booking1.html', default_context)
...
def bookflexible(request):
    ...
    print(existing_booking_slot1)
    print(existing_booking_slot2)
    print(existing_booking_slot3)
    print(existing_booking_slot4)
    ...
    if request.method == 'POST':
        ...
        start_datetime = datetime.combine(current_date, start_time)
        start_datetime_str = str(start_datetime)
        end_datetime = datetime.combine(datetime.today(), time(23, 59, 0))
...
def show_slot_availability(request):
    ...
    available_slots1 = Booking.objects.filter(selected_slot = "Slot-1").exclude(status = 'Ended')
    available_slots2 = Booking.objects.filter(selected_slot = "Slot-2").exclude(status = 'Ended')
    available_slots3 = Booking.objects.filter(selected_slot = "Slot-3").exclude(status = 'Ended')
    available_slots4 = Booking.objects.filter(selected_slot = "Slot-4").exclude(status = 'Ended')
    
    
    if available_slots1.exists():
        
        time_free = Booking.objects.filter(selected_slot = "Slot-1", start_time__date__gte=timezone.now().date()).order_by("start_time")
        list_for_slot1 = []
    ...
    if available_slots2.exists():
        ...
        time_free = Booking.objects.filter(selected_slot = "Slot-2", start_time__date__gte=timezone.now().date()).order_by("start_time")
        list_for_slot2 = []
        for booking in time_free:
            start_time = booking.start_time.strftime("%I:%M %p")
            end_time = booking.end_time.strftime("%I:%M %p")
    ...
    if available_slots3.exists():
        ...
        time_free = Booking.objects.filter(selected_slot = "Slot-3", start_time__date__gte=timezone.now().date()).order_by("start_time")
        list_for_slot3 = []
        for booking in time_free:
            start_time = booking.start_time.strftime("%I:%M %p")
            end_time = booking.end_time.strftime("%I:%M %p")
    ...
    if available_slots4.exists():
        ...
        time_free = Booking.objects.filter(selected_slot = "Slot-4", start_time__date__gte=timezone.now().date()).order_by("start_time")
        list_for_slot4 = []
        for booking in time_free:
            start_time = booking.start_time.strftime("%I:%M %p")
            end_time = booking.end_time.strftime("%I:%M %p")
    ...
    if request.method == 'POST':
        ...
        if 'file' in request.FILES:
            ...
            if text:
                ...
                print('NNNNNNNNN')
                if active_booking:
                    print(active_booking.start_time)
                print('NNNNNNNNN')
                active_booking_flex = Booking.objects.filter(
                    ...
                    booking_type='flexible',
                    start_time__lte=current_time, 
                    end_time=datetime.combine(datetime.today(), time(23, 59, 0)),
                ).first()
                print('NNNNNNNNN2')
                if active_booking_flex:
                    print(active_booking_flex.start_time)
                print('NNNNNNNNN2')
                ...
                print('NNNNNNNNN3')
                if active_booking1_flex:
                    print(active_booking1_flex.start_time)
                print('NNNNNNNNN3')
                if active_booking_flex:
...
def esewa(request): #problem
    ...
    start_datetime_str = request.GET.get('start_time')
    end_datetime_str = request.GET.get('end_time')
    booking_type_value = request.GET.get('booking_type')
    if booking_type_value=='fixed':
        buffer_time = timedelta(minutes=15)
    elif booking_type_value=='flexible':
        buffer_time = timedelta(minutes=0)
    total_endtime_str = request.GET.get('total_endtime')
    print('1----------------------------------------')
    print( total_endtime_str)
    if '?' in total_endtime_str:
        print('2----------------------------------------')
        total_endtime_str = total_endtime_str.split('?')[0]
        print('3----------------------------------------')
    print('4----------------------------------------')
    print( total_endtime_str)
    payment_status = 'Paid'
...
def paymment_method(request):
    return render(request, 'payment_esewa.html')
def paymment_methodflex(request):
    return render(request, 'payment_esewa_flex.html')
...
def payment_success(request, booking_id):
    ...
    if request.method == 'POST':
        if booking:
            ...
            booking_iid = booking_id
            user = request.user
            user_id = user.id
            difference_in_minutes = time_difference.total_seconds() / 60
            total = difference_in_minutes//60
            if total ==0:
                total = 1
            context = {
                'fullname':full_name,
...
# Get the difference in minutes
    ...
    return render(request, 'payment_esewa_flex.html', context)

def esewa_flex(request):
    userid =request.GET.get('userid')
    user = get_object_or_404(User, id=int(userid))
    bookingid =request.GET.get('bookingid')
    booking = get_object_or_404(Booking, id=int(bookingid), user=user)
    booking.payment = 'Paid'
    booking.save()
    fullname = request.GET.get('full_name')
